...
extension SHCircleBar {
    ...
    private func drawPath(for index: CGFloat) -> UIBezierPath {
        ...
        let leftPointCurveDown = CGPoint(
            x: (((index * tabWidth) - tabWidth*0.2) + tabWidth / 4)+35,
            y: tabHeight*0.40)
        
        let middlePoint = CGPoint(
            x: ((tabWidth * index) + tabWidth / 2) + 4,
            y: tabHeight*0.4)
        let middlePointCurveDown = CGPoint(
            x: ((((index * tabWidth) - tabWidth*0.2) + tabWidth / 10) + tabWidth) - 30,
            y: tabHeight*0.40)
        let middlePointCurveUp = CGPoint(
            x: (((tabWidth * index) + tabWidth) - tabWidth / 3),
            y: 0)
        let rightPoint = CGPoint(x: ((tabWidth * index) + tabWidth) - 15 , y: 0)
